	...
	<div class="card-body p-8 bg-white rounded border border-secondary">
		<div class="d-flex justify-content-between mb-10">
			<div class="card-title m-0">
				<h3 class="fw-bold m-0">Liste Conducteurs</h3>
			</div>
			<a href="{{ route('conducteurs_create') }}" class="btn btn-sm btn-primary align-self-center">Ajouter Conducteure</a>
		</div>
		...
		@else
				...
				<tbody class="fw-semibold text-gray-600 text-center">
					@foreach ( $conducteurs as $key => $conducteur )
						<tr>
							...
							<td>
								<a href="#" class="text-gray-800 text-hover-primary fs-5 fw-bold">{{ $conducteur->nom }}</a>
							</td>
							<td>
								<span>{{ $conducteur->prenom }}</span>
							</td>
							<td>
								<span>{{ $conducteur->lage }}</span>
							</td>
							<td>
								<span>{{ $conducteur->n_cin }}</span>
							</td>
							<td>
								<span>{{ Str::limit($conducteur->adresse, 15, '...') }}</span>
							</td>
							<td>
								<span class="fw-bold">{{ $conducteur->telephone }}</span>
							</td>
							<td>
								...
								<div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
									...
									<div class="menu-item px-3">
										<a href="{{ route('conducteurs_show', $conducteur->id) }}" class="menu-link px-3">View</a>
									</div>
									<!--end::Menu item-->
									<!--begin::Menu item-->
									<div class="menu-item px-3">
										<a href="{{ route('conducteurs_edit', $conducteur->id) }}" class="menu-link px-3">Edit</a>
									</div>
									<!--end::Menu item-->
									<!--begin::Menu item-->
									<div class="menu-item px-3">
										<a href="{{ route('conducteurs_destroy', $conducteur->id) }}" class="preconfirmed menu-link px-3">Delete</a>
									</div>
									<!--end::Menu item-->
								</div>
								<!--end::Menu-->
			...
			<div class="d-flex justify-content-center">
				{!! $conducteurs->links('pagination::bootstrap-4') !!}
			</div>
		@endif
		<!--end::Table-->
	</div>
